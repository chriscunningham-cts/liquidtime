...
def handle_errors(response, message):
    ...
    raise Exception(f"{message}: {failure_code}\n" f"{failure_message}")
...
def add_timesheet_entry(
    ...
    click.echo(f"Added to timesheet {postresult['timesheet_entry']['timesheet_id']}.")
    return postresult["timesheet_entry"]["timesheet_id"]


def submit_timesheet(workspace_id, token, timesheet_id, confirm):
    timesheet_submit_url = (
        f"{api_path}/workspaces/{workspace_id}/timesheets/{timesheet_id}/submit"
    )
    if not confirm and not click.confirm(
...
def get_timesheet_entries(ctx, date):
    timesheets_url = (
        f"{api_path}/workspaces/{ctx.obj['workspace_id']}/timesheet_entries"
    )
    params = {"member_id": get_member_id(ctx.obj["token"])}
    if date:
        params["start_date"] = params["end_date"] = date
    timesheets = handle_errors(
        requests.get(timesheets_url, headers=headers(ctx.obj["token"]), params=params),
...
def load_config(ctx, config, append, confirm):
    ...
    if not unique_timesheets:
        ...
        for timesheet_id in unique_timesheets:
            submit_timesheet(
                ctx.obj["workspace_id"], ctx.obj["token"], timesheet_id, confirm
            )
